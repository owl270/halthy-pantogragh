...
import {SET} from "../../DataHandler/_actions";
    ...
    render() {
        ...
        if (!calendar_range[0] || !start_duration) {
            ...
            const $trips = this.props.device_trips.map((item, ii) => {
                ...
                return item.trips.map((trips) => {


                    const $st = moment(trips['start']).diff(calendar_range[0], 'hours') / 24
                    const $en = moment(trips['end']).diff(calendar_range[0], 'hours') / 24
